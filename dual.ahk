class DualHotKey {
	...
	settings := {delay: 0, timeout: 200, doublePress: 200, specificDelays: false}
	...
	__New(settings:=false) {
        if (settings != false){
            this.override(this.settings, settings)
        }
	}
	...
	combine(downKey, upKey, settings:=false, combinators:=false) {
        ...
        if (SubStr(currentKey, -2) = "UP"){
            this.keyup(keys, currentKey, lastKey)
        } else {
            this.keydown(keys, currentKey, lastKey)
        }
	}
	...
	combo(currentKey, justReleasedDownKeyTimeDown:=-1) {
		shorterTimeDownKeys := []
		for originalKey, keys in this.keys.OwnProps() {
			upKey := keys.upKey
	...
	reset() {
		this.keys := {}
		for index, modifier in this.allModifiers.OwnProps() {
	...
	SendAny(string, mode:="") {
		...
		for index, downKey in temporarilyReleasedKeys.OwnProps() {
			downKey.down()
		}
	}
	...
	getKeysFor(currentKey, downKey, upKey, settings, combinators) {
		...
		if (ObjHasOwnProp(this.keys, cleanKey)) {
			keys := this.keys.%cleanKey%
		} else {
			keys := KeyPair(downKey, upKey, this.settings, settings, combinators)
	...
	keydown(keys, currentKey, lastKey) {
		...
		if (ObjHasOwnProp(upKey, "repeatMode") and upKey.repeatMode == true) {
			upKey.send()
			return
		}
	...
	keyup(keys, currentKey, lastKey) {
		...
		if ((not ObjHasOwnProp(downKey, "combo") or downKey.combo == false)
			and (downKeyTimeDown < keys.timeout or keys.timeout == -1)
			and (not ObjHasOwnProp(upKey, "alreadySend") or upKey.alreadySend == false)) {
			...
			if (shorterTimeDownKeys != false) { ; The exception referred to above.
				...
				for index, keys in shorterTimeDownKeys.OwnProps() {
					keys.abortDualRole()
				}
			}
		}
	...
	sendInternal(string) {
		if (ObjHasOwnProp(Dual, "sentKeys")) {
	...
	override(master, extension, options:=false) {
		if (IsObject(options) and ObjHasOwnProp(options, "onto")) {
			overrided := options.onto
			for key, value in master.OwnProps() {
				overrided.%key% := value
			}
		} else {
			overrided := master
        ...
        if (IsObject(extension)){
            for key, value in extension {
                if (not ObjHasOwnProp(master, key)) {
	...
	subKeySet(key) {
		...
		for index, subKey in key.OwnProps() {
...
class KeyPair {
    __New(downKey, upKey, defaults, settings, combinators) {
        this.downKey := Key(downKey)
        this.upKey   := Key(upKey, combinators)
        Dual.override(defaults, settings, {onto: this})

		if (IsObject(settings) and ObjHasOwnProp(settings, "specificDelays") and IsObject(settings.specificDelays) and ObjHasOwnProp(settings.specificDelays, "extend")) {
            this.specificDelays.Remove("extend")
            for keySet, delay in defaults.specificDelays.OwnProps() {
                if (not ObjHasOwnProp(this.specificDelays, keySet)) {
        ...
        if (IsObject(this.specificDelays)){
            for keySet, delay in this.specificDelays.OwnProps() {
...
class Key {
    ...
    down(sendActualKeyStrokes:=true) {
        ...
        for index, key in this.key { ; (*)
            ...
            if ((ObjHasOwnProp(this.subKeysDown, key) and this.subKeysDown.%key%) or not GetKeyState(key)) {
    ...
    up(sendOnly:=false) {
        ...
        for index, key in this.key { ; (*)
            ...
            ; been called.
            if (ObjHasOwnProp(this.subKeysDown, key)) {
